...
import { usePlayerStats, useSeasonId, useSeasonLeaders, useSetPlayerStats } from '../hooks'
...
export function PlayerProfile() {
  ...
  const playerMeta = useMemo(() => {
    ...
    const metaByPlayer: Record<string, { characters: string[]; flavor: string }> = {
      martin: { characters: ['Rauw'], flavor: 'The Cliff Diver' },
      niklas: { characters: ['Ungeork'], flavor: 'The Cliff Diver' },
      seb: { characters: ['Nudelkriger'], flavor: 'The Cliff Diver' },
      niko: { characters: ['Freecry'], flavor: 'The Cliff Diver' },
    }

    const meta = metaByPlayer[key]
    return (
      meta ?? {
        characters: playerStats?.player.displayName ? [playerStats.player.displayName] : [],
        flavor: 'The Cliff Diver',
      }
    )
  }, [playerStats])
            ...
            <header className="relative flex flex-col gap-2xl overflow-hidden rounded-3xl border border-accent-secondary bg-surface-section p-2xl shadow-2xl sm:flex-row sm:items-start sm:justify-between">
              <div className="flex min-w-0 flex-1 items-start gap-lg">
                ...
                <div className="min-w-0 flex-1 pt-xs">
                  ...
                  <p className="pt-sm text-sm leading-5 text-stat-yeets">{playerMeta.flavor}</p>

                  <p className="pt-sm text-sm leading-5 text-text-secondary">
                    {playerMeta.characters.length} characters tracked this season
                  </p>
              ...
              <NemesisCard
                ...
                bannerImageUrl={(() => {
                  const seasonKey = /Season\s*2/i.test(playerStats.season.name)
                    ? 'season2'
                    : 'season1'
                  return getDungeonBannerImageFromStats(seasonKey, nemesis.dungeon)
                })()}
              />
            ) : null}
