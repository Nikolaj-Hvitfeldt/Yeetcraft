...
import type { DungeonLeaderboardEntry, DungeonSummary, SeasonLeadersResponse } from '../api/types'
import type { DungeonMistakeMix, DungeonReputationScores } from './dungeon-stats'
...
export type DungeonAchievementHolderView = NonNullable<DungeonAchievement['holder']>
...
function getContributors(leaderboard: DungeonLeaderboardEntry[]): DungeonLeaderboardEntry[] {
  return leaderboard.filter((entry) => entry.totalMistakes > 0)
}

function getYeetLeader(leaderboard: DungeonLeaderboardEntry[]): DungeonLeaderboardEntry | null {
  return pickLeader(
    leaderboard,
    (entry) => entry.yeets,
    (entry) => entry.deaths,
...
function getDeathLeader(leaderboard: DungeonLeaderboardEntry[]): DungeonLeaderboardEntry | null {
  return pickLeader(
    leaderboard,
    (entry) => entry.deaths,
    (entry) => entry.yeets,
...
function pickHighestYeetRatioPlayer(
  ...
  const eligible = leaderboard.filter(
    (entry) => entry.totalMistakes > 0 && entry.yeets / entry.totalMistakes >= YEET_RATIO_THRESHOLD,
  )
...
function pickHighestDeathRatioPlayer(
  ...
  const eligible = leaderboard.filter(
    (entry) =>
      entry.totalMistakes > 0 && entry.deaths / entry.totalMistakes >= DEATH_RATIO_THRESHOLD,
  )
  if (eligible.length === 0) return null
...
function pickMostBalancedPlayer(
  ...
  const eligible = leaderboard.filter(
    (entry) => entry.deaths > 0 && entry.yeets > 0 && Math.abs(entry.deaths - entry.yeets) <= 1,
  )
  if (eligible.length === 0) return null
...
function pickBenchwarmer(leaderboard: DungeonLeaderboardEntry[]): DungeonLeaderboardEntry | null {
...
const ACHIEVEMENT_RULES: AchievementRule[] = [
  ...
  },
  {
    ...
    evaluate: ({ leaderboard, seasonLeaders }) => {
      ...
      const suspects = [
        seasonLeaders?.kingOfYeets && yeetLeader?.playerId !== seasonLeaders.kingOfYeets.playerId
          ? seasonLeaders.kingOfYeets
          : null,
        seasonLeaders?.kingOfDeaths && deathLeader?.playerId !== seasonLeaders.kingOfDeaths.playerId
      ...
      const uniqueSuspects = sortByName(
        suspects.filter(
          (king, index, kings) =>
            kings.findIndex((entry) => entry.playerId === king.playerId) === index,
        ),
      )
  ...
  },
  {
    ...
    evaluate: ({ leaderboard }) => {
      ...
      const yeetPercent = Math.round((player.yeets / player.totalMistakes) * 100)
      return {
        eligible: true,
        holder: toHolder(player),
  ...
  },
  {
    ...
    evaluate: ({ leaderboard }) => {
      ...
      const deathPercent = Math.round((player.deaths / player.totalMistakes) * 100)
      return {
        eligible: true,
        holder: toHolder(player),
...
function evaluateRules(context: DungeonAchievementContext): ScoredAchievement[] {
...
function selectTopAchievements(candidates: ScoredAchievement[]): DungeonAchievement[] {
  ...
  const canTake = (candidate: ScoredAchievement) => {
    ...
    if (candidate.holderPlayerId && usedHolderIds.has(candidate.holderPlayerId)) {
      return false
    }
    return true
  }
  ...
  if (selected.length === 1 && selected[0]?.hasHolder) {
    const dungeonAward = sorted.find((candidate) => canTake(candidate) && !candidate.hasHolder)
...
export function getDungeonAchievements(context: DungeonAchievementContext): DungeonAchievement[] {
