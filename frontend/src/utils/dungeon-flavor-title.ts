...
import { getFlavorDescription, type FlavorDescriptionContext } from './dungeon-flavor-description'
...
function compareDungeonTieBreak(first: DungeonSummary, second: DungeonSummary): number {
...
function pickLowestScoringDungeon(
  ...
  const eligible = dungeons.filter(
    (dungeon) =>
      !excludedDungeonIds.has(dungeon.id) && (isEligible ? isEligible(dungeon, context) : true),
  )
  let winner: DungeonSummary | null = null
  ...
  },
  {
    ...
    pickWinner: (dungeons, context, excludedDungeonIds) =>
      pickHighestScoringDungeon(
        dungeons,
        excludedDungeonIds,
        (dungeon, titleContext) => titleContext.blameShareByDungeonId.get(dungeon.id) ?? 0,
        context,
        70,
      ),
  },
  ...
  },
  {
    ...
    pickWinner: (dungeons, context, excludedDungeonIds) =>
      pickLowestScoringDungeon(
        dungeons,
        excludedDungeonIds,
        (dungeon, titleContext) => titleContext.blameShareByDungeonId.get(dungeon.id) ?? 100,
        context,
        (dungeon, titleContext) => {
          const blameShare = titleContext.blameShareByDungeonId.get(dungeon.id) ?? 0
          return (
...
export function assignUniqueDungeonTitles(
  ...
  for (const rule of UNIQUE_DUNGEON_TITLE_RULES) {
    const winner = rule.pickWinner(input.allDungeons, context, claimedDungeonIds)
    if (!winner) continue
...
function getFallbackDungeonTitle(dungeon: DungeonSummary, allDungeons: DungeonSummary[]): string {
  ...
  if (seasonAverageMistakes > 0 && dungeon.totalMistakes > seasonAverageMistakes * 1.1) {
    return 'The Punching Bag'
  }

  if (seasonAverageMistakes > 0 && dungeon.totalMistakes < seasonAverageMistakes * 0.9) {
...
function buildDescription(title: string, input: DungeonFlavorTitleInput): string {
  const context =
    ...
    ({
      ...
      contributorCount: 0,
      cleanPlayerCount: 0,
      seasonAverageMistakes: Number(getAverageMistakesPerDungeon(input.allDungeons).toFixed(1)),
      yeetSharePercent:
        input.dungeon.totalMistakes > 0
          ? Math.round((input.dungeon.totalYeets / input.dungeon.totalMistakes) * 100)
          : 0,
      deathSharePercent:
        input.dungeon.totalMistakes > 0
          ? Math.round((input.dungeon.totalDeaths / input.dungeon.totalMistakes) * 100)
          : 0,
...
export function getDungeonFlavorTitle(input: DungeonFlavorTitleInput): DungeonFlavorTitleResult {
  const uniqueTitles = assignUniqueDungeonTitles(input)
  const title =
    uniqueTitles.get(input.dungeon.id) ?? getFallbackDungeonTitle(input.dungeon, input.allDungeons)
