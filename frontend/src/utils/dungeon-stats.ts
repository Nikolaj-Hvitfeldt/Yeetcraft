import type { DungeonLeaderboardEntry, DungeonMistakeLeader, DungeonSummary } from '../api/types'
...
export function getYeetFactor(dungeon: DungeonSummary, allDungeons: DungeonSummary[]): number {
  ...
  const totalSeasonYeets = allDungeons.reduce((sum, entry) => sum + entry.totalYeets, 0)
  const totalSeasonMistakes = allDungeons.reduce((sum, entry) => sum + entry.totalMistakes, 0)
...
function getDangerVerdict(dangerRating: number, dungeonTotalMistakes: number): string {
...
function getBlameVerdict(blameShare: number, dungeonTotalMistakes: number): string {
  if (dungeonTotalMistakes <= 0) {
...
function buildFlavorDescriptionContext(
  ...
  return {
    ...
    seasonAverageMistakes: Number(getAverageMistakesPerDungeon(allDungeons).toFixed(1)),
...
export function getMeatGrinderSummary(
  ...
  const flavorTitle = getDungeonFlavorTitle({
    ...
    descriptionContext: buildFlavorDescriptionContext(dungeon, leaderboard, allDungeons),
  })
