...
import { getBlameShare, getDungeonReputationScores, getMistakeMix } from './dungeon-stats'
...
function buildContext(
  ...
  return {
    ...
    leaderboard,
    reputationScores: getDungeonReputationScores(activeDungeon, allDungeons, leaderboard),
...
describe('getDungeonAchievements', () => {
  it('picks liability and benchwarmer when one player owns a messy run', () => {
    const achievements = getDungeonAchievements(buildContext(soloOffenderLeaderboard))
  ...
  it('solo act rule includes the sole contributor as holder', () => {
    const soloActRule = dungeonAchievementRulesForTests.find((rule) => rule.id === 'solo-act')
    ...
    expect(result.description).toBe('Every mistake in this dungeon came from Niklas')
  })
  ...
  it('prefers a dungeon award as the second pick when the first has a holder', () => {
    ...
    const achievements = getDungeonAchievements({
      ...
      reputationScores: getDungeonReputationScores(spreadDungeon, allDungeons, spreadLeaderboard),
  ...
  it('dungeon menace rule matches the season mistake leader for this key', () => {
    const menaceRule = dungeonAchievementRulesForTests.find((rule) => rule.id === 'dungeon-menace')
    ...
    expect(result.eligible).toBe(true)
    expect(result.description).toBe('Niklas leads mistakes for this dungeon')
  })
  ...
  it('the usual suspect rule fires when a season king did not top this dungeon', () => {
    ...
    expect(result.eligible).toBe(true)
    expect(result.description).toBe('Niklas usually tops the charts — not here')
  })

  it('carry job rule fires for a single clean player in a messy run', () => {
    const carryRule = dungeonAchievementRulesForTests.find((rule) => rule.id === 'carry-job')
